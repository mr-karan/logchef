...
// handleListTeamSourceBookmarkedQueries lists all bookmarked queries for a specific team and source.
// Assumes requireAuth and requireTeamMember middleware have run.
func (s *Server) handleListTeamSourceBookmarkedQueries(c *fiber.Ctx) error {
	teamIDStr := c.Params("teamID")
	sourceIDStr := c.Params("sourceID")
	...
	queries, err := s.sqlite.ListBookmarkedQueriesByTeamAndSource(c.Context(), teamID, sourceID)
	if err != nil {
		return SendErrorWithType(c, fiber.StatusInternalServerError, "Failed to list bookmarked queries", models.GeneralErrorType)
	}

	return SendSuccess(c, fiber.StatusOK, queries)
}

// handleCreateTeamSourceBookmarkedQuery creates a new bookmarked saved query (collection) for a specific team and source.
// Assumes requireAuth and requireTeamMember middleware have run.
func (s *Server) handleCreateTeamSourceBookmarkedQuery(c *fiber.Ctx) error {
	teamIDStr := c.Params("teamID")
	sourceIDStr := c.Params("sourceID")
...
// handleGetTeamSourceBookmarkedQuery retrieves a specific saved query (collection) belonging to a team/source.
// Assumes requireAuth and requireTeamMember middleware have run.
func (s *Server) handleGetTeamSourceBookmarkedQuery(c *fiber.Ctx) error {
	teamIDStr := c.Params("teamID")
	sourceIDStr := c.Params("sourceID")
	bookmarkedQueryIDStr := c.Params("bookmarkedQueryID")
	if bookmarkedQueryIDStr == "" {
		return SendErrorWithType(c, fiber.StatusBadRequest, "Bookmarked Query ID is required", models.ValidationErrorType)
	}
	...
	bookmarkedQueryID, err := strconv.Atoi(bookmarkedQueryIDStr)
	if err != nil {
		return SendErrorWithType(c, fiber.StatusBadRequest, "Invalid Bookmarked Query ID format", models.ValidationErrorType)
	}

	// Call the sqlite method directly for now due to linter issues with core.handleNotFoundError
	query, err := s.sqlite.GetTeamSourceQuery(c.Context(), teamID, sourceID, bookmarkedQueryID)
	if err != nil {
		// LINTER_ISSUE: Cannot reliably check core.ErrQueryNotFound due to potential resolution issues.
		// Checking for sql.ErrNoRows from the underlying db call instead.
		if errors.Is(err, sql.ErrNoRows) { // Check for underlying DB error
		...
		s.log.Error("failed to get bookmarked query via db function", slog.Any("error", err), "bookmarked_query_id", bookmarkedQueryID)
		return SendErrorWithType(c, fiber.StatusInternalServerError, "Failed to retrieve bookmarked query", models.GeneralErrorType)
	}

	return SendSuccess(c, fiber.StatusOK, query)
}

// handleUpdateTeamSourceBookmarkedQuery updates a saved query (bookmarked query).
// Assumes requireAuth, requireTeamMember, and requireTeamAdminOrGlobalAdmin middleware have run.
func (s *Server) handleUpdateTeamSourceBookmarkedQuery(c *fiber.Ctx) error {
	teamIDStr := c.Params("teamID")
	sourceIDStr := c.Params("sourceID")
	bookmarkedQueryIDStr := c.Params("bookmarkedQueryID")
	if bookmarkedQueryIDStr == "" {
	...
	if err != nil {
		...
		s.log.Error("failed to update bookmarked query via db function", slog.Any("error", err), "bookmarked_query_id", bookmarkedQueryID)
		return SendErrorWithType(c, fiber.StatusInternalServerError, "Failed to update bookmarked query", models.GeneralErrorType)
	}

	// Fetch and return updated query.
	updatedQuery, err := s.sqlite.GetTeamSourceQuery(c.Context(), teamID, sourceID, bookmarkedQueryID)
	if err != nil {
		s.log.Error("failed to fetch updated bookmarked query after update", slog.Any("error", err), "bookmarked_query_id", bookmarkedQueryID)
		return SendErrorWithType(c, fiber.StatusInternalServerError, "Bookmarked query updated but failed to retrieve latest state", models.GeneralErrorType)
	}

	return SendSuccess(c, fiber.StatusOK, updatedQuery)
}

// handleDeleteTeamSourceBookmarkedQuery deletes a saved query (bookmarked query).
// Assumes requireAuth, requireTeamMember, and requireTeamAdminOrGlobalAdmin middleware have run.
func (s *Server) handleDeleteTeamSourceBookmarkedQuery(c *fiber.Ctx) error {
	teamIDStr := c.Params("teamID")
	sourceIDStr := c.Params("sourceID")
	bookmarkedQueryIDStr := c.Params("bookmarkedQueryID")
	if bookmarkedQueryIDStr == "" {
	...
	// Call sqlite delete function.
	// Middleware ensures user has appropriate team admin rights.
	err = s.sqlite.DeleteTeamSourceQuery(c.Context(), teamID, sourceID, bookmarkedQueryID)
	if err != nil {
		// DELETE often doesn't return ErrNoRows.
		if errors.Is(err, sql.ErrNoRows) { // Check just in case.
			return SendErrorWithType(c, fiber.StatusNotFound, "Collection not found", models.NotFoundErrorType)
		}
		s.log.Error("failed to delete bookmarked query via db function", slog.Any("error", err), "bookmarked_query_id", bookmarkedQueryID)
		return SendErrorWithType(c, fiber.StatusInternalServerError, "Failed to delete bookmarked query", models.GeneralErrorType)
	}

	return SendSuccess(c, fiber.StatusOK, fiber.Map{"message": "Bookmarked query deleted successfully"})
}