...
// caller supplies a fully-formed model; scope serialization happens here.
func (s *Store) CreateAPIToken(ctx context.Context, token *models.APIToken) (int, error) {
	scopes, err := marshalTokenScopes(token.Scopes)
	if err != nil {
		return 0, err
	}
	id, err := s.q.CreateAPIToken(ctx, sqlc.CreateAPITokenParams{
		UserID:    int64(token.UserID),
		Name:      token.Name,
		TokenHash: token.TokenHash,
		Prefix:    token.Prefix,
		ExpiresAt: tsFromPtr(token.ExpiresAt),
		Scopes:    scopes,
	})
	if err != nil {
		s.log.Error("failed to create API token record in db", "error", err, "user_id", token.UserID)
...
func (s *Store) GetAPIToken(ctx context.Context, id int) (*models.APIToken, error) {
	row, err := s.q.GetAPIToken(ctx, int64(id))
	if err != nil {
		if notFound(err) {
			return nil, models.ErrNotFound
		}
		s.log.Error("failed to get API token from db", "error", err, "token_id", id)
...
func (s *Store) GetAPITokenByHash(ctx context.Context, tokenHash string) (*models.APIToken, error) {
	row, err := s.q.GetAPITokenByHash(ctx, tokenHash)
	if err != nil {
		if notFound(err) {
			return nil, models.ErrNotFound
		}
		s.log.Error("failed to get API token by hash from db", "error", err)
...
func (s *Store) ListAPITokensForUser(ctx context.Context, userID models.UserID) ([]*models.APIToken, error) {
	rows, err := s.q.ListAPITokensForUser(ctx, int64(userID))
	if err != nil {
		s.log.Error("failed to list API tokens for user from db", "error", err, "user_id", userID)
...
func (s *Store) UpdateAPITokenLastUsed(ctx context.Context, id int) error {
	if err := s.q.UpdateAPITokenLastUsed(ctx, int64(id)); err != nil {
		s.log.Error("failed to update API token last used timestamp", "error", err, "token_id", id)
		return fmt.Errorf("failed to update API token last used: %w", err)
	}
	return nil
}

// DeleteAPIToken deletes an API token, scoped to its owner.
func (s *Store) DeleteAPIToken(ctx context.Context, id int, userID models.UserID) error {
	err := s.q.DeleteAPIToken(ctx, sqlc.DeleteAPITokenParams{ID: int64(id), UserID: int64(userID)})
	if err != nil {
		s.log.Error("failed to delete API token from db", "error", err, "token_id", id, "user_id", userID)
...
func (s *Store) DeleteExpiredAPITokens(ctx context.Context) error {
	if err := s.q.DeleteExpiredAPITokens(ctx); err != nil {
		s.log.Error("failed to delete expired API tokens from db", "error", err)
		return fmt.Errorf("failed to delete expired API tokens: %w", err)
	}
	return nil
}

// apiTokenToModel maps a stored row to the model, decoding the scopes JSON and
// computing the Expired flag from the current clock.
