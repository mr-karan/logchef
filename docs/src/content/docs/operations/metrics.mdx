...
description: Logchef's Prometheus metrics for monitoring usage and performance
---

Logchef exposes Prometheus metrics covering instance usage, performance, and health: query patterns, user activity, authentication events, and resource use.
...
- Build the dashboards above inside Logchef itself: see [Dashboards](/features/dashboards)
- Review [Database Backends & High Availability](/operations/database-backends) if you're scaling to multiple replicas