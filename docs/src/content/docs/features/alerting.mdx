...
`enabled` is the **server-wide** alerting switch. With `alerts.enabled = false`
(or `LOGCHEF_ALERTS__ENABLED=false`) Logchef hides all alerting UI and every
alert endpoint returns `503` — useful for exploration-only deployments, or to
keep alert evaluation on a single instance when running multiple replicas
against a shared [Postgres backend](/operations/database-backends/).
