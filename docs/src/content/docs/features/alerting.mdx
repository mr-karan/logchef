...
1. Create an alert with a SQL query and threshold (e.g., "error count > 100")
...
**In LogChef:**
1. Navigate to your team/source
2. Create alert: "High API Errors"
3. Query: `SELECT count(*) FROM logs WHERE severity = 'ERROR' AND timestamp >= now() - INTERVAL 5 MINUTE`
4. Threshold: Greater than 100
5. Frequency: Every 60 seconds
6. Save
...
## Viewing Alert Status and History

### Where to Find Your Alerts

**Alert List View:**
- Navigate to your team → source → Alerts tab
- See all alerts for that source
- Status indicators show if alert is currently triggered
- Quick access to edit, delete, or view history
