...
For ClickHouse sources, Logchef can generate SQL automatically from a LogchefQL condition. For VictoriaLogs sources, alerts currently use native LogsQL only.

## Query Modes

**Condition mode (ClickHouse)** — Write a LogchefQL filter condition and pick an aggregate function. Logchef generates SQL and adds the time filter automatically.
...
**Native mode** — Write the source's native alert query. The query must return a single numeric column named `value`.
...
For VictoriaLogs sources, native mode uses LogsQL and evaluates through `stats_query`.
...
| Query | ClickHouse LogchefQL condition, ClickHouse SQL, or VictoriaLogs LogsQL depending on source |
