...
description: "Non-login service accounts with scoped API tokens for automation and CI: add them to teams and control which sources and actions each token can access."
---
...
- Use a token with the [CLI](/integration/cli/#api-token-authentication) for scripted queries
- Pass a token to the [MCP Server](/integration/mcp-server) for AI assistant access
- Review [Declarative Provisioning](/getting-started/provisioning) to manage teams and sources as code
