...
tail stream directly, so new rows arrive as a genuine push rather than a poll.
Live tail is available in LogchefQL mode and in native LogsQL mode; the time-range
and limit controls are disabled while a tail is running.
