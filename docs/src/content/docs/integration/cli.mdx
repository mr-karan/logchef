...
| `--source` | `-S` | Source name (or ID) | (from config) |
...
:::tip[Name Resolution]
The CLI caches team/source name lookups locally (`~/.cache/logchef/`) for 10 minutes, so repeated queries are fast. You can also use numeric IDs if you prefer.
:::
