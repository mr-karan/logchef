---
title: Logchef CLI
description: Query ClickHouse and VictoriaLogs logs from the terminal with the Logchef CLI, built for scripting and quick investigations.
---

A Rust CLI for querying logs from the terminal. Supports LogchefQL across supported sources, raw native queries (SQL for ClickHouse, LogsQL for VictoriaLogs), syntax highlighting (via [tailspin](https://github.com/bensadeh/tailspin)), multi-context management (dev/staging/prod), and OIDC PKCE authentication.
...
Grab the latest version tag from the [CLI releases page](https://github.com/mr-karan/logchef/releases?q=cli&expanded=true) (e.g. `0.2.1`), set it once, then download:
...
:::tip[Match the CLI to your server]
Run `logchef doctor` after installing to confirm the CLI and server versions line up. A mismatch is a warning, not a hard failure.
:::
...
Generate API tokens from the Logchef web UI under your profile settings, or see
[Service Tokens](/features/service-tokens) for non-login automation accounts.

## Command Reference

:::note[Works on both engines]
`query`, `sql`, `tail`, `find`, `explain`, `fields`, and `histogram` all run against ClickHouse **and** VictoriaLogs sources. LogchefQL (`query`, `tail`) compiles to whichever language the source speaks; `sql` sends ClickHouse SQL or LogsQL verbatim; `explain` shows exactly what a filter compiles to. For a VictoriaLogs walkthrough, see the [VictoriaLogs tutorial](/tutorials/victorialogs/).
:::

### Query Logs

The `query` command runs LogchefQL queries from your terminal.
...
- Use `query` (LogchefQL) for quick filtering across supported sources with convenient time flags (`--since 1h`)
- Use `sql` when you need full control over the source's native query language
:::
...
:::tip[`explain` vs `--explain`/`--show-sql`]
`explain` is a translate-and-validate step that **never executes**. The `--explain` / `--show-sql` flags on `query` and `sql` trace the generated backend query on stderr but **still run** it. Use `explain` when you only want to see or check the query; use the flags when you want the results and the query.
:::
...
:::tip[For agents]
If you're driving Logchef from an AI agent or script, run `logchef skills get core` first. The output is version-matched to the binary, so the flags and behaviors it describes are guaranteed to exist.
:::
...
The CLI highlights common log patterns automatically, using [tailspin](https://github.com/bensadeh/tailspin).
...
- Learn the filter syntax the `query` command accepts in [Search Syntax](/guide/search-syntax)
- Query a VictoriaLogs source end to end in the [VictoriaLogs tutorial](/tutorials/victorialogs/)
- Connect an AI assistant to the same data with the [MCP Server](/integration/mcp-server)
- Configure the `oidc.cli_client_id` prerequisite in [Configuration](/getting-started/configuration/#openid-connect-oidc)
