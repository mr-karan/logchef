...
A Rust CLI for querying logs from the terminal. Supports LogchefQL across supported sources, raw ClickHouse SQL, syntax highlighting (via [tailspin](https://github.com/bensadeh/tailspin)), multi-context management (dev/staging/prod), and OIDC PKCE authentication.
