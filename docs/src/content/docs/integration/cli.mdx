...
logchef query "" --output jsonl --no-highlight > logs.jsonl
...
```bash
logchef query "status:500" --output jsonl --no-highlight > errors.jsonl
```

### Count errors with jq

```bash
logchef query "level:error" --output json --no-highlight | jq 'length'
```
