...
The [Logchef MCP server](https://github.com/mr-karan/logchef-mcp) lets AI assistants query logs, explore schemas, manage alerts, and administer your Logchef deployment through the [Model Context Protocol](https://modelcontextprotocol.io/). It also provides read-only resources and guided investigation prompts.
...
2. **An API token**: generate one from your Logchef profile settings page, or issue a scoped [service token](/features/service-tokens) for the assistant. The token determines what data the MCP server can access.
...
Tools are organized by capability. Each one includes MCP annotations indicating whether it is read-only, non-destructive, or destructive.
...
| Tool | Description |
|------|-------------|
| `generate_query` | Generate source-native ClickHouse SQL or LogsQL from natural language (requires AI enabled on Logchef) |
...
- Prefer a terminal workflow? See the [Logchef CLI](/integration/cli)
- Review [Service Tokens](/features/service-tokens) for scoping what the assistant can access
- Check [AI SQL Generation](/features/ai-sql-generation) for the web UI's built-in AI assistant
